...
@Component({
  selector: 'app-pointstrackingreportt',
  templateUrl: './pointstrackingreportt.component.html',
  styleUrls: ['./pointstrackingreportt.component.scss']
})
export class PointstrackingreporttComponent implements OnInit {
  ...
  refreshReport(): void {
    debugger;
    this.isLoading = true;
    this.service.executeGenericReportData('getPointsTrackingReport',{ContactId: this.selectedContacts, fromDate: this.fromDate, toDate: this.toDate}).subscribe({
      next: (data: any) => {
        if(data.length > 0){
