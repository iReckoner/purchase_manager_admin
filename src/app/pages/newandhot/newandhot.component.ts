...
@Component({
  selector: 'app-newandhot',
  templateUrl: './newandhot.component.html',
  styleUrls: ['./newandhot.component.scss']
})
export class NewandhotComponent implements OnInit {
  dataSource: any[] = [];
  ...
  loadData() {
    debugger
    this.loading = true;
    this.service.executeData('NewAndHot').subscribe({
      next: (res: any) => {
  ...
  onAddClick() {
    debugger;
    this.router.navigate(['/newandhot', 'new']);
  }
  ...
  onEditClick(e: any) {
    ...
    if (e.row) {
      ...
      tabId = e.row.data.tabId;
    } else if (e.event && e.rowIndex >= 0) {
      // If called from edit button
      tabId = this.dataSource[e.rowIndex].tabId;
    } else if (e.data) {
    ...
    if (tabId) {
      this.router.navigate(['/newandhot', 'edit', tabId]);
    }
  }
