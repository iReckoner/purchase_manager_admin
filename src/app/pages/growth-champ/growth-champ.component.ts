...
@Component({
  selector: 'app-growth-champ',
  templateUrl: './growth-champ.component.html',
  styleUrls: ['./growth-champ.component.scss']
})
export class GrowthChampComponent implements OnInit {
  ...
  refreshReport(): void {
    ...
    this.service.growthChamp(payload).subscribe({
      next: (data: any) => {
        if(data.length > 0){
          this.reportData = data;
          this.selectedRowKeys = [];
          const selectedRows = this.reportData.filter(x => x.isCheck == true);
  ...
  updateReport(): void {  
    ...
    const data = { 
      data: { 
        "GrowthChamp": [updatedRow]  // Wrap in array to match expected API format
      } 
    };
