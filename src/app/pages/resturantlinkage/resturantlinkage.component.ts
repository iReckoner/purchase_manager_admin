...
@Component({
  selector: 'app-resturantlinkage',
  templateUrl: './resturantlinkage.component.html',
  styleUrls: ['./resturantlinkage.component.scss']
})
export class ResturantlinkageComponent  implements OnInit {
  dataSource: any[] = [];
  ...
  loadData() {
    this.loading = true;
    this.service.executeData('resturantlinkage').subscribe({
      next: (res: any) => {
  ...
  onEditClick(e: any) {
    ...
    PartyId = e.data.PartyId;

    if (PartyId) {
      this.router.navigate(['/resturantlinkages', 'edit', PartyId]);
    }
  }
