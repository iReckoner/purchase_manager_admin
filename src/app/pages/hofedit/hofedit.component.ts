...
@Component({
  selector: 'app-hofedit',
  templateUrl: './hofedit.component.html',
  styleUrls: ['./hofedit.component.scss']
})
export class HofeditComponent implements OnInit {
  @Input() tabId?: number;
  ...
  amountEditorOptions: any = {
    ...
    onValueChanged: (e: any) => {
      if (e.value !== null && e.value !== undefined) {
        e.component.option('value', parseFloat(e.value).toFixed(2));
      }
    }
  };
  ticketAmountEditorOptions: any = {
    disabled: false,
    format: {
      type: 'fixedPoint',
      precision: 2
  ...
  loadEvent(): void {
    ...
    this.loading = true;
    this.service.getDocmentInfo2('HofMaster', {Id: this.tabId})
      .subscribe({
        next: (res: any) => {
          if (res && res.length > 0) {
  ...
  onSaving(): void {
    ...
    if(+this.currentEvent['Ticket_Amount'] < 0){
      notify("Ticket Amount can't be less than zero", 'error', 3000);
      return;
    }
    ...
    const data = { data: { "HallOfFame": contactData } };
    ...
    apiCall$.subscribe({
      next: (response: any) => {
        ...
        this.router.navigate(['/hofreport']);
      },
      error: (error: any) => {
