...
@Component({
  selector: 'app-sku-rockstart',
  templateUrl: './sku-rockstart.component.html',
  styleUrls: ['./sku-rockstart.component.scss']
})
export class SkuRockstartComponent implements OnInit {
  ...
  refreshReport(): void {
    ...
    this.service.skuRockstar(payload).subscribe({
      next: (data: any) => {
        if(data.length > 0){
  ...
  updateReport(): void {  
    ...
    const data = { data: { "SKURockstar":  [updatedRow]} };
