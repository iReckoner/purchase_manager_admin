...
export class BannerComponent implements OnInit {
  ...
  loading = false;
  apiUrl: any;
  types:any[]=[{name: 'MEETS', id: 'MEETS'},{name: 'PARTIES', id: 'PARTIES'},{name: 'TRAINING', id: 'TRAINING'},{name: 'PICNICS', id: 'PICNICS'}];
  currentEvent: any = {
    imageUrl: '',
  ...
  uploadImage(file: File): void {
    if (!this.tabId) {
      notify('Event ID is required', 'error', 3000);
      return;
    }

    const formData = new FormData();
    formData.append('file', file);
    const fileName = `Events_${this.tabId}.PNG`;
    ...
    this.service.uploadImage(formData, 'Images', fileName).subscribe({
      next: (response: any) => {
        ...
        this.eventImageUrl = `${this.apiUrl}/images/viewImage/Events_${this.tabId}.PNG?t=${timestamp}`;
  ...
  ngOnInit(): void {
    ...
    this.service.getDocmentInfo2('BannerMaster', {})
      .subscribe({
        next: (res: any) => {
          if (res && res.length > 0) {
  ...
  onSaving(): void {
    ...
    const apiCall$ = this.tabId 
      ? this.service.updateContact(this.tabId, data)
